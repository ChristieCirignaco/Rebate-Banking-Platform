...
export const getMenu = cache(async (location: MenuLocation): Promise<MenuLink[]> => {
  let rows: { label: string | null; url: string | null; openInNew: boolean; isActive: boolean; page: { title: string; path: string; isActive: boolean } | null }[];
  try {
  ...
  for (const row of rows) {
    if (!row.isActive) continue; // deactivated item → hidden (curated-empty menu is a valid admin choice)
    if (row.page) {
