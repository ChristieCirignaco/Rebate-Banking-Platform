  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <section className="flex flex-col gap-2">
        <h2 className="text-sm font-semibold text-slate-900 dark:text-slate-100">Your referral link</h2>
        <div className="flex items-center gap-2 rounded-xl border border-slate-200 bg-slate-50/70 p-1.5 pl-3.5 dark:border-slate-800 dark:bg-slate-800/50">
        ...
        <p className="text-xs text-slate-500 dark:text-slate-400">
          <span className="font-semibold text-slate-700 dark:text-slate-200">{data.joinCount}</span>{" "}
      ...
      <section className="overflow-hidden rounded-2xl border border-slate-200 dark:border-slate-800">
        <button
          ...
          className="flex w-full items-center gap-3 p-4 text-left transition-colors hover:bg-slate-50 disabled:cursor-default disabled:hover:bg-transparent dark:hover:bg-slate-800"
        >
          <Avatar size="lg" className="ring-2 ring-blue-100 dark:ring-blue-500/30">
            {data.self.image ? <AvatarImage src={data.self.image} alt={data.self.name} /> : null}
            <AvatarFallback className="bg-blue-50 text-sm font-semibold text-blue-700 dark:bg-blue-500/10 dark:text-blue-400">
            ...
            {data.referrals.map((r) => (
              <div key={r.id} className="flex items-center gap-3 py-2.5 pr-4 pl-6">
                <Avatar size="default">
                  ...
                  <AvatarFallback className="bg-slate-100 text-xs font-semibold text-slate-600 dark:bg-slate-800 dark:text-slate-400">
                    {initials(r.name)}
                  </AvatarFallback>
                </Avatar>
                <div className="min-w-0 flex-1">
                  <p className="truncate text-sm font-medium text-slate-900 dark:text-slate-100">{r.name}</p>
                  <p className="text-xs text-slate-400 dark:text-slate-500">{r.joinedLabel}</p>
                </div>
      ...
      <section className="flex flex-col gap-3">
        <h2 className="text-sm font-semibold text-slate-900 dark:text-slate-100">Program rules</h2>
        <div className="grid gap-3 sm:grid-cols-2">
          <div className="rounded-2xl border border-emerald-100 bg-emerald-50/50 p-4 dark:border-emerald-500/30 dark:bg-emerald-500/10">
            <p className="mb-2 text-xs font-semibold tracking-wide text-emerald-700 uppercase dark:text-emerald-400">
              Allowed
            </p>
          ...
          <div className="rounded-2xl border border-red-100 bg-red-50/40 p-4 dark:border-red-500/30 dark:bg-red-500/10">
      ...
      <section className="flex flex-col gap-3">
        <div className="flex items-center justify-between">
          <h2 className="text-sm font-semibold text-slate-900 dark:text-slate-100">Referral earnings</h2>
          <div className="flex items-center gap-3 text-xs">
            <span className="text-slate-500 dark:text-slate-400">
              Paid <span className="font-semibold text-emerald-600 dark:text-emerald-400">{data.totalEarnedLabel}</span>
            </span>
            <span className="text-slate-500 dark:text-slate-400">
              Pending <span className="font-semibold text-amber-600 dark:text-amber-400">{data.pendingLabel}</span>
            </span>
          </div>
        </div>

        {data.earnings.length === 0 ? (
          <div className="flex flex-col items-center gap-2 rounded-2xl border border-dashed border-slate-200 py-10 text-center dark:border-slate-800">
            <Gift className="size-6 text-slate-300 dark:text-slate-500" />
            <p className="text-sm text-slate-400 dark:text-slate-500">No referral earnings yet.</p>
            ...
            <div className="overflow-x-auto">
              <table className="w-full min-w-[420px] text-left text-sm">
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                  {data.earnings.map((e) => (
                    <tr key={e.id}>
                      <td className="px-4 py-3 font-medium text-slate-900 dark:text-slate-100">{e.referredName}</td>
                      <td className="px-4 py-3 font-medium tabular-nums text-slate-900 dark:text-slate-100">
                        {e.amountLabel}
                      </td>
                      <td className="px-4 py-3 text-xs text-slate-500 dark:text-slate-400">{e.dateLabel}</td>
