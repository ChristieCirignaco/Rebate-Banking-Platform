    ...
    <div className="flex flex-col gap-4 pb-2">
      ...
      {data.total ? (
        <div className="flex items-baseline justify-between rounded-2xl bg-slate-50 px-4 py-3">
          <span className="text-sm text-slate-500">Total in {data.total.currency}</span>
          <span className="text-base font-bold text-slate-900">{data.total.amountLabel}</span>
        </div>
      ) : null}
...
function WalletCard({ wallet }: { wallet: WalletCardView }) {
  return (
    <section className="flex flex-col gap-3 rounded-2xl border border-slate-200 p-4 transition-colors hover:border-slate-300">
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            ...
            {wallet.isDefault ? (
              <span className="shrink-0 rounded-full bg-blue-50 px-2 py-0.5 text-[10px] font-semibold tracking-wide text-blue-600 uppercase">
                Primary
              </span>
            ) : null}
          </div>
